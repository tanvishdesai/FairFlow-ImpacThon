"""
Run order-stress robustness tests for the upgraded FairFlow method.
"""
...
from research.elite_runner import OrderStressConfig, run_order_stress_benchmark
from research.elite_methods import PrimalDualOffsetConfig


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run order-stress tests for FairFlow.")
    parser.add_argument("--search-root", action="append", default=["/kaggle/input", "data/raw"])
    parser.add_argument("--output-dir", default="research_outputs/elite_order_stress")
    parser.add_argument("--datasets", nargs="+", default=["adult", "compas", "recruitment"])
    parser.add_argument("--protocols", nargs="+", default=["natural", "alternating_groups", "privileged_first", "unprivileged_first"])
...
def main() -> None:
    ...
    results, traces = run_order_stress_benchmark(
        search_roots=args.search_root,
        output_dir=args.output_dir,
        config=config,
        show_rl_progress=args.show_rl_progress,
    )
    print(f"Saved {len(results)} order-stress rows to {args.output_dir}/per_run_results.csv")
    print(f"Saved {len(traces)} order-stress trace rows to {args.output_dir}/rolling_traces.csv")
